...
void AI::setSignature(playField_t& board)
{
    ...
    else
    {
        ...
        for (int i{ 1 }; i < static_cast<int>(aiBoard.size()); ++i) //move through cells
            if (aiBoard[i] != Signature::x && aiBoard[i] != Signature::o) //ie empty
            {
                ...
                int score{ minimax(aiBoard, false) }; //play as human
                aiBoard[i] = Signature::clear; //undo position
                if (score > best)
                {
                    best = score;
                    index = i;
    ...
    //finalized routine
    std::cout << "Choosing position " << index << '\n';
...
int AI::checkGlobalWinner(playField_t& board)
{
    ...
    return 0; //all checks are done => tied state probably
}

int AI::minimax(playField_t& state, bool isMax) //depth not implemented
{
    //state is the copy of the real gameboard
    ...
    if (isMax) //ai turn
    {
        ...
        for (int i{ 1 }; i < static_cast<int>(state.size()); ++i) //check all cells
        {
            if (state[i] != Signature::x && state[i] != Signature::o) //ie clear
            {
                ...
                best = std::max(best, minimax(state, false)); //play as human and get Max value
    ...
    else //human turn
    {
        ...
        for (int j{ 1 }; j < static_cast<int>(state.size()); ++j) //check all cells
        {
            if (state[j] != Signature::x && state[j] != Signature::o) //ie clear
            {
                ...
                best = std::min(best, minimax(state, true)); //play as ai
